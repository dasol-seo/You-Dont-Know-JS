...
#### 비동등
 ...
 `<`, `>`, `<=`, `>=` 연산자는 비동등에 사용됩니다, 기술명세서에 "관계 비교"라고 언급되어 있습니다. 일반적으로 `number`와 같은 서수 비교에 사용되므로, `3 < 4`를 쉽게 이해할 수 있을 것입니다.

하지만 자바스크립트의 `string`값은 일반적인 알파벳 규칙을 사용하므로 비동등로도 비교가 가능합니다 (`"bar" < "foo"`).
...
잠재적으로 타입이 다른 값들을 비교할 때 실수할 수 있는것은 둘 중 하나의 값이 유요한 숫자로 변환할 수 없는 경우 입니다 --"엄격한 비동등"형식을 사용하지 않는다는것을 기억하세요.-- 예를 들면:
...
**Note:** 더 많은 비동등 규칙을 알고 싶으시다면 ES5의 기술명세서 중 11.8.5 부분을 확인해 보세요. 또한 Types & Grammar시리즈의 4장을 참고하세요.
