...
방금 말한 내장타입과 하위타입은 매우 강력하고 유용한 속성와 메소드를 갖고 있습니다.
...
속성 또는 메소드 (예를 들어 이전의 스니펫에서 `a.toUpperCase()`)를 참조하는 `object`처럼 여러분이 `"hello world"`과 같은 원시값을 사용할 때, 자바스크립트는 자동으로 값을 객체 래퍼(덮개 아래 숨겨진)로 "감쌉니다".
...
여러분은 아마 "강제 변환은 악마다"와 같은 말을 들어봤을 겁니다. 강제 변환이 특정 위치에서는 예상치 못 한 결과를 불러올 수 있기 때문에 전해진 말 입니다. 아마 언어가 당황스럽게 할 때만큼 개발자들이 좌절감을 느끼는 때도 없을 것 입니다.
