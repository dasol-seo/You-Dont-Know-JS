...
접근 불가능한 스코프 안에서 변수의 값에 접근을 시도한다면, `ReferenceError`를 받게 될 것입니다. 선언이 되어지지 않은 곳에서 변수에 설정을 하려고 하면 "엄격 모드"("엄격 모드"를 보세요) 에 따라 결국 변수가 최상단 전역 스코프에 만들어 지거나 (좋지 않음!) 에러가 발생할것입니다. 함께 봅시다: 
