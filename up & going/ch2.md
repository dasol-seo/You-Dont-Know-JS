...
대괄호 방식은 만약 여러분이 지정한 속성의 이름이 특수 문자 등을 포함하고 있을 때 (ex. `obj["hello world!"]` --) 유용하게 사용할 수 있습니다. 이러한 속성들의 이름은 해당 객체에 대괄호 방식으로 접근할 때 보통 *keys*라고 불립니다. `[ ]` 방식은 변수(다음에 설명합니다.)나 `string` *리터럴*(`" .. "` 나 `' .. '`로 감싸져 있는)을 필요로 합니다.
...
자바스크립트 프로그램에서 일반적으로 상호작용하는 몇 가지 다른 값 타입들이 있습니다: *배열*와 *함수*. 하지만 이 두 타입은 자바스크립트에 내장된 타입이라고 하기보단, `object` 타입에서 조금 더 전문화되어 만들어진 버전, 일종의 서브 타입이라고 생각하는 것이 좋습니다.
